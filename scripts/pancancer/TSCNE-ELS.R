...
ccre.enh.p1 <- ccre.enh %>% filter(encodeLabel == "pELS", contain_G4 == 1) %>% GrData()
ccre.enh.p0 <- ccre.enh %>% filter(encodeLabel == "pELS", contain_G4 == 0) %>% GrData()
ccre.enh.d1 <- ccre.enh %>% filter(encodeLabel == "dELS", contain_G4 == 1) %>% GrData()
ccre.enh.d0 <- ccre.enh %>% filter(encodeLabel == "dELS", contain_G4 == 0) %>% GrData()

tscre.map <- fread("../data/TSCRE/H3K27ac/TSCRE_ID_H3K27ac.txt", sep = "\t", header = FALSE) %>% data.frame()
all.files <- list.files("../data/TSCRE/H3K27ac/", recursive = TRUE, pattern = "*-Annot.txt$", full.names = TRUE)
all.files <- data.frame(path = all.files, ID = str_split(all.files, "(/|CRE_)", simplify = TRUE)[, 7])
tscre.map2 <- fread("../data/TSCRE/H3K4me3/TSCRE_ID_H3K4me3.txt", sep = "\t", header = FALSE) %>% data.frame()
...
for (tmp.ct in all.ct) {
  ...
  p1.mat <- normalizeToMatrix(tmp.tscre, ccre.enh.p1, value_column = "score",
                              extend = extend.width, mean_mode = "coverage", w = width)
  gc()
  p0.mat <- normalizeToMatrix(tmp.tscre, ccre.enh.p0, value_column = "score",
                              extend = extend.width, mean_mode = "coverage", w = width)
  gc()
  d1.mat <- normalizeToMatrix(tmp.tscre, ccre.enh.d1, value_column = "score",
                              extend = extend.width, mean_mode = "coverage", w = width)
  gc()
  d0.mat <- normalizeToMatrix(tmp.tscre, ccre.enh.d0, value_column = "score",
                              extend = extend.width, mean_mode = "coverage", w = width)
  gc()
  window <- extend.width/width
  els.mat <- bind_rows(els.mat, bind_rows(
                                  data.frame(value = colMeans(p1.mat), group = "G4 cCREs", cCRE = "pELS", range = c(-window:window), ct = tmp.ct),
                                  data.frame(value = colMeans(p0.mat), group = "non-G4-associated cCREs", cCRE = "pELS", range = c(-window:window), ct = tmp.ct),
                                  data.frame(value = colMeans(d1.mat), group = "G4 cCREs", cCRE = "dELS", range = c(-window:window), ct = tmp.ct),
                                  data.frame(value = colMeans(d0.mat), group = "non-G4-associated cCREs", cCRE = "dELS", range = c(-window:window), ct = tmp.ct)
                                  ))

}

save(list=ls(), file = "./TSCNE-ELS.RData")
